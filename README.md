...
1.  **Desktop Commander (DC):** An MCP server (or CLI) that gives the agent real filesystem access, terminal control, and safety guardrails. It allows agents to read/write files and run tests.
2.  **Repomix:** Packs repository context into optimized prompt bundles. Instead of dumping your entire codebase into the chat (hallucinations + high cost), Repomix lets you surgically include *only* the files relevant to the current task.
3.  **The `.ai/` Directory:** The brain of the project. Every document that matters lives here.
4.  **Gemini CLI / Claude Desktop:** The reasoning engine. Interchangeable logic processors.
...
# Optional: Install Gemini CLI (`gemini auth`) for free-tier usage
```
